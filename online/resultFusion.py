...
def fusion():
    print ("run fusion...")
    differAllData = pd.read_csv('differData.csv', names=['uid', 'cid', 'value'])
    changedData = pd.read_csv('changedBigraph.csv',names=['uid', 'cid', 'value'])
    ...
    dvalue = mergeData['value_x'].values.tolist()
    cvalue = mergeData['value_y'].values.tolist()
    fvalue = map(lambda(a,b,c):a*0.3+b+c,zip(cvalue,dvalue,norm_ctlist))
    fvalue = np.array(fvalue)
    fvalue = np.round(fvalue,2)
    fvalue = list(fvalue)

    #mergeData增量数据变为进行融合后的结果集
    mergeData.drop(['value_x','value_y'],axis=1,inplace=True)
    mergeData['value'] = fvalue
    print mergeData
